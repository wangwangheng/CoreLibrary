...
//        expandedTabLayoutParams = new LinearLayout.LayoutParams(200, LayoutParams.MATCH_PARENT);
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            for (int j = 0; j < frameLayout.getChildCount(); j++) {
                ...
                if (v instanceof TextView) {
                    ...
                    View normal = tabViews.get(i).get("normal");
                    View selected = tabViews.get(i).get("selected");
                    ...
                    if (i == selectedPosition) {
                        ...
                        View n = tabViews.get(i).get("normal");
                        View s = tabViews.get(i).get("selected");
...
//        canvas.drawRect(lineLeft, height - indicatorHeight, lineRight, height, rectPaint);
        ...
        for (int i = 0; i < tabCount - 1; i++) {
            ...
            canvas.drawLine(tab.getRight(), dividerPaddingTopBottom,
                    tab.getRight(), height - dividerPaddingTopBottom, dividerPaint);
        }
    }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            View oldNormal = tabViews.get(oldPosition).get("normal");
            View oldSelected = tabViews.get(oldPosition).get("selected");
            if (oldNormal != null) {
                oldNormal.setAlpha(1);
            }
            if (oldSelected != null) {
                oldSelected.setAlpha(0);
            }
            ...
            View normal = tabViews.get(position).get("normal");
            View selected = tabViews.get(position).get("selected");
    ...
    public void setUnderlineHeightPx(int underlineHeightPx) {
        this.underlineHeight = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_PX, underlineHeightPx, getResources().getDisplayMetrics());
        invalidate();
    }
...
//        notifyDataSetChanged();
    ...
    public void setShouldExpandByRequest(boolean shouldExpand) {
    ...
    protected void animateFadeScale(View left, View right, float positionOffset, int position) {
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                View selected = tabViews.get(position).get("selected");
                if (normal != null) {
                    normal.setAlpha(positionOffset);
                }
                if (selected != null) {
                    selected.setAlpha(1 - positionOffset);
                }
            ...
            if (right != null) {
                View normal = tabViews.get(position + 1).get("normal");
                View selected = tabViews.get(position + 1).get("selected");
                if (normal != null) {
                    normal.setAlpha(1 - positionOffset);
                }
                if (selected != null) {
                    selected.setAlpha(positionOffset);
                }
